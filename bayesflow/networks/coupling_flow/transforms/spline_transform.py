...
class SplineTransform(Transform):
    def __init__(self, bins: int = 32, domain: tuple[float, float, float, float] = (-5.0, 5.0, -5.0, 5.0), **kwargs):
        super().__init__(**kwargs)
        self.bins = bins
        self.input_edges = keras.ops.linspace(domain[0], domain[1], bins + 1)
        self.output_edges = keras.ops.linspace(domain[2], domain[3], bins + 1)

    def bin_parameters(self, parameters: Tensor) -> list[Tensor]:
        return list(keras.ops.split(parameters, self.bins, axis=-1))

    def _forward(self, x: Tensor, parameters: dict[str, Tensor]) -> (Tensor, Tensor):
        _bins = keras.ops.searchsorted(self.input_edges, x)
        ...
        ...
        # default_width = (self.domain[1] - self.domain[0]) / self.bins
        # default_height = (self.domain[3] - self.domain[2]) / self.bins
